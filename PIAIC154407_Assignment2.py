...
def function2():
    ...
    x =  np.float64(np.arange(10, 37).reshape(3, 3, 3))
...
def function9():
    ...
    a = np.linspace(2, 100, num=49, dtype='int64')
...
def function12():
    ...
    arr = np.array([1,2,3])
    ans = np.concatenate((np.repeat(arr, 3, axis = 0), np.tile(arr, 3)))
...
def function18():
    ...
    x = np.count_nonzero((np.isin(piaic, students))== True)# Write you code Here
    return x
